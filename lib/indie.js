...
class ProjectLinter {
  ...
  runSelectedScan() {
    ...
    if (!scanItems.length) {
      lumine.notifications.addWarning("TODO selected scan skipped", {
  ...
  //
  // Directories go through `lumine.project.crawl()` so the editor's ripgrep does
  ...
  async collectFiles(targetPaths, ignoredNames, ignoreGlob) {
    ...
    if (directoryPaths.length) {
      await lumine.project.crawl({
        directoryPaths,
        ignoredNames,
        excludeVcsIgnoredPaths: lumine.config.get("linter-todo.excludeVcsIgnoredPaths"),
  ...
  async runScan(scanItems = null) {
    ...
    const resolvedScanItems = scanItems || lumine.project.getPaths();
    ...
    const ignoredNames = lumine.config.get("linter-todo.ignoredNames") || [];
    ...
    const task = Task.once(
      ...
      () => {
        ...
        lumine.notifications.addWarning("TODO project scan failed", {
